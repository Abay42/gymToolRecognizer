...
def get_gymtool_by_name(db: Session, name: str) -> GymTool:
    ...
    if tool:
        ...
        logger.info(f"Associated muscles: {[muscle.name for muscle in tool.muscles]}")
...
def add_muscle_to_gymtool(db: Session, gymtool_name: str, muscle_name: str):
    logger.info(f"Adding muscle {muscle_name} to gym tool {gymtool_name}")

    gym_tool = get_gymtool_by_name(db, gymtool_name)
    muscle = get_muscle_by_name(db, muscle_name)
    ...
    if muscle in gym_tool.muscles:
        logger.warning(f"Muscle '{muscle_name}' is already linked to '{gymtool_name}'")
        return None

    gym_tool.muscles.append(muscle)
    db.commit()
    logger.info(f"Added muscle '{muscle_name}' to gym tool '{gymtool_name}'")
    return gym_tool
